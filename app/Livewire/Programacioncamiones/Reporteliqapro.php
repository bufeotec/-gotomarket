...
//    }
    ...
    public function exportarDespachosExcel()
    {
        try {
            ...
            // ========== TABLA LOCAL (LIMA) ==========
            $sheet->setCellValue('A3', 'DESPACHO LOCAL (LIMA)');
            ...
            if ($localData->isNotEmpty()) {
                ...
                // Total General Local con bordes en negrita
                $sheet->setCellValue('A'.$row, 'TOTAL GENERAL');
                $sheet->mergeCells('A'.$row.':E'.$row);
                $sheet->setCellValue('F'.$row, 'S/ ' . number_format($localData->sum('sin_igv'), 2));
                $sheet->setCellValue('H'.$row, 'S/ ' . number_format($localData->sum('con_igv'), 2));
            ...
            if ($provincialData->isNotEmpty()) {
                ...
                foreach ($groupedDataProvincial as $proveedor => $guias) {
                    ...
                    foreach ($guias as $guia) {
                        ...
                        $sheet->setCellValue('L'.$rowProvincial, ($guia->departamento ?? 'SIN DEPARTAMENTO').'-'.($guia->provincia ?? 'SIN PROVINCIA'));
                        $sheet->setCellValue('M'.$rowProvincial, $proveedor);
                        $sheet->setCellValue('N'.$rowProvincial, $guia->fact ?? '---');
                        $sheet->setCellValue('O'.$rowProvincial, $guia->sin_igv); // Valor numérico sin formato
                        $sheet->setCellValue('P'.$rowProvincial, $guia->con_igv); // Valor numérico sin formato

                        if ($firstRow) {
                            $sheet->setCellValue('Q'.$rowProvincial, $totalProveedorConIgv); // Valor numérico sin formato
                            $firstRow = false;
                        }

                        // Centrar solo las columnas requeridas
                        $sheet->getStyle('L'.$rowProvincial)->getAlignment()->setHorizontal('center');
                        $sheet->getStyle('N'.$rowProvincial)->getAlignment()->setHorizontal('center');
                        $rowProvincial++;
                    }
                }

                // Aplicar formato numérico a las columnas O, P, Q
                $sheet->getStyle('O5:O'.$rowProvincial)->getNumberFormat()->setFormatCode('"S/"#,##0.00');
                $sheet->getStyle('P5:P'.$rowProvincial)->getNumberFormat()->setFormatCode('"S/"#,##0.00');
                $sheet->getStyle('Q5:Q'.$rowProvincial)->getNumberFormat()->setFormatCode('"S/"#,##0.00');

                // Total General Provincial con bordes en negrita
                $sheet->setCellValue('J'.$rowProvincial, 'TOTAL GENERAL');
                $sheet->mergeCells('J'.$rowProvincial.':N'.$rowProvincial);
                $sheet->setCellValue('O'.$rowProvincial, $provincialData->sum('sin_igv'));
                $sheet->setCellValue('Q'.$rowProvincial, $provincialData->sum('con_igv'));
                ...
                $sheet->getStyle('O'.$rowProvincial)->getNumberFormat()->setFormatCode('"S/"#,##0.00');
                $sheet->getStyle('Q'.$rowProvincial)->getNumberFormat()->setFormatCode('"S/"#,##0.00');
                $sheet->getStyle('J'.$rowProvincial)->getAlignment()->setHorizontal('center');
            } else {
