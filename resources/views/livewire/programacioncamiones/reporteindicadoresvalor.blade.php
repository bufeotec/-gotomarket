    ...
    <div class="row">
        <div class="col-lg-12 col-md-6 col-sm-12 mb-2">
            <div>
                ...
                <button class="btn bg-primary text-white" wire:click="buscar_reporte_valor">
                    <i class="fa fa-search"></i> BUSCAR
                </button>
                @if(count($filteredData) > 0)
                    <button class="btn bg-success text-white" wire:click.prevent="exportarReporteValorExcel">
        ...
        <div class="col-lg-2 col-md-6 col-sm-12 mb-2">
            ...
            <input type="month" wire:model.live="mxdesde" class="form-control">
        </div>

        <div class="col-lg-2 col-md-6 col-sm-12 mb-2">
            <label class="form-label">M. Hasta</label>
            <input type="month" wire:model.live="mxhasta" class="form-control">
        </div>
    ...
    @if($searchdatos && count($filteredData) > 0)
        <div class="row">
            <div class="col-lg-5 col-md-12 col-sm-12 mb-4">
                <x-card-general-view>
                    <x-slot name="content">
                        <h6 class="text-center mt-2 mb-3">OBJETIVO DE INDICADORES</h6>
                        <x-table-general>
                            <x-slot name="thead">
                                <tr>
                                    <th>Zona de Despacho</th>
                                    <th class="text-center">Flete / Valor (%)</th>
                                </tr>
                            </x-slot>
                            <x-slot name="tbody">
                                @foreach(['Total','Lima', 'Provincia 1', 'Provincia 2'] as $zona)
                                    <tr>
                                        <td>{{ $zona }}</td>
                                        <td class="text-center">{{ $summary[$zona]['porcentaje'] ?? 0 }}%</td>
                                    </tr>
                                @endforeach
                            </x-slot>
                        </x-table-general>
            ...
            <div class="col-lg-7 col-md-12 col-sm-12 mb-4">
                <x-card-general-view>
                    <x-slot name="content">
                        <h6 class="text-center mt-2 mb-3">REPORTE RESUMEN</h6>
                        <x-table-general>
                            <x-slot name="thead">
                                <tr>
                                    <th>Zona de Despacho</th>
                            ...
                            <x-slot name="tbody">
                                @foreach(['Total','Lima', 'Provincia 1', 'Provincia 2'] as $zona)
                                    <tr>
                                        <td>{{ $zona }}</td>
                                        <td class="text-center">S/ {{ number_format($summary[$zona]['flete'] ?? 0, 2) }}</td>
                                        <td class="text-center">S/ {{ number_format($summary[$zona]['valor'] ?? 0, 2) }}</td>
                                        <td class="text-center">{{ $summary[$zona]['porcentaje'] ?? 0 }}%</td>
                                    </tr>
                                @endforeach
                            </x-slot>
